async function productsList (){
    const conexao = await fetch("https://lojinha-iota.vercel.app/products");
    const conexaoConvertida = await conexao.json();
    return { conexaoConvertida, statusConexao: conexao.status };
}

async function searchProducts(searchTerm) {
    const conexao = await fetch("https://lojinha-iota.vercel.app/products"+searchTerm);
    const conexaoConvertida = await conexao.json();
    return { conexaoConvertida, statusConexao: conexao.status };
}
...
async function editProduct (id, image, name, description, price, promocionalPrice, seller, category, tag01, color01, color02, color03, size01, size02, size03, size04, size05){
    ...
    const conexao = await fetch("https://lojinha-iota.vercel.app/products"+id, {
        method: "PATCH",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({
            product_image_desktop: image,
...
async function deleteProduct(productId) {
    const conexao = await fetch("https://lojinha-iota.vercel.app/products"+productId, {
        method: "DELETE",
        headers: {"Content-Type": "application/json"},
    });
    const statusConexao = conexao.status;
